...
const MessageForm = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      await StudentServices.createStudentMessage(conversationId, {
        ...data,
      });
    } catch (error) {
  ...
  return (
    <div className="dark:bg-[#151515] py-4 border-gray-300  px-4 bg-white flex items-center w-full border-t gap-2 lg:gap-4 dark:rounded-sm dark:border-t-0 dark:border-x dark:border-b dark:border-gray-800">
      ...
      >
        <Plus size={23} className="text-pink-500 hover:text-pink-800" />
      </CldUploadButton>
      <form
      ...
      >
        ...
        <button
          type="submit"
          className="rounded-full p-2 bg-pink-500 hover:bg-pink-800 text-white transition cursor-pointer"
        >
